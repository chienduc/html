	...
	if(frmObj.length > 0){
		...
		function initFormElements(els){
			for(var i=0; i<els.length; i++){												
				var elObj = frmObj.find('[id="'+ els[i].field +'"]');
				if(elObj.length > 0){
		...
		function isValidElement(el){				
			var pos_error = -1;
			var elObj = frmObj.find('[id="'+ el.field +'"]');
		...
		function showLayer(el, msg){			
			var coords = frmObj.find('[id="'+ el +'"]').offset();		
			var layer = frmObj.data('alertForm').removeClass('hidden');
			frmObj.find('[id="'+ el +'"]').css('border-color','#ff0000');
			layer.find('.message').text(msg);
			layer.css({
				'top': coords.top + frmObj.find('[id="'+ el +'"]').outerHeight(),
				'left': coords.left
			});
 ...
 */
 function showPopupLayer(id, keepmask){
	...
	$('body').unbind().bind('click', function(event){
		...
		if(!$target.parents().is(id) && !$target.is(id)){
			if(keepmask == 'keepmask'){
				hideLayer(id, keepmask);
			}else{
				hideLayer(id);		
	...
	if(arrChk.length > 0){
		arrChk.each(function(index, rad){			
			if($(rad).hasClass('rdchecked')){
				prev = $(rad);
				$(rad).find(':first').attr('checked','checked');
			}			
			$(rad).unbind('click').bind('click', function(e){
				e.preventDefault();				
				if(!$(this).hasClass('rdchecked')){
					$('.methodContent').slideUp();
					jQuery('.method').removeClass('nobd');
					jQuery(this).parent().addClass('nobd');
				...
				if(prev == null){
					...
					if($(this).find('input').length > 0){
						$(this).find(':first').attr('checked','checked');
					}
 ...
 */ (function (b) {
    ...
    b.smoothScroll = function (c, e) {
        ...
        if (typeof c === "number") {
            ...
            g = e || b(a.scrollTarget)[d]() && b(a.scrollTarget)[d]()[a.direction] || 0
        }
        a = b.extend({
            link: null
        }, a);
