...
include_once(ROOT_PATH."classes/dao/staffcategoryinfo.class.php");
...
class StaffCategories extends Model {
	...
	function StaffCategories ($store_id = 0, $database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		} else $this->_db = $database;
		$this->table = DB_PREFIX."staff_categories";
...
*-----------------------------------------------------------------------*/	
	...
	function cleanTrash() {
		...
		if($results) {
			...
			$staff = new Staff($this->store_id);
			# Loop all DELETED categories
			foreach($results as $key => $result) {
				# Change status of all products in each category to DELETED too
				$staff->update(array('status' => S_DELETED),"`store_id` = '".$this->store_id."' AND `cat_id` = '".$result['id']."'");
			}	
		}
	...
	# Return a StaffCategory Id from provided ID
	...
	# Return a StaffCategory Name from provided slug
	...
	# Return a StaffCategory slug from provided ID
	...
	# Return a StaffCategory name from provided ID
...
# Return StaffCategory from provided parent_id
