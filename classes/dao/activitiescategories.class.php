...
include_once(ROOT_PATH.'classes/dao/activitiescategoryinfo.class.php');

class ActivitiesCategories extends Model {
	var $table;
	var $_db;
	var $store_id;
	
	function ActivitiesCategories($store_id = 0,$database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		...
		$this->table = DB_PREFIX."activities_categories";
...
*-----------------------------------------------------------------------*/
	function getObject($value = '0', $key = 'id', $condition = '1>0') {
		...
		if($result) {
			$object = new ActivitiesCategoryInfo(
									$result[0]['store_id'],
									$result[0]['name'],
									$result[0]['content'],
									$result[0]['status'],
	...
	function getObjects($page = 1, $condition = '1>0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$objects[] = new ActivitiesCategoryInfo(
									$result['store_id'],
									$result['name'],
									$result['content'],
									$result['status'],
