...
include_once(ROOT_PATH.'classes/dao/campaigninfo.class.php');
...
class Campaign extends Model {
	...
	function Campaign($store_id = 0,$database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
...
*-----------------------------------------------------------------------*/
	function getObject($value = '0', $key = 'id', $condition = '1>0') {
		...
		if($result) {
			$object = new CampaignInfo(
									$result[0]['name'],
									$result[0]['email'],
									$result[0]['name_display'],
