...
class ProductCategoryInfo {
	...
	# Constructor
	function ProductCategoryInfo($slug, $name, $keyword, $sapo, $position, $viewed, $properties, $status,$home, $store_id = 0, $parent_id = 0, $id = 0)
	{
		$this->id = $id;
	...
	function getListProducts() {
		include_once(ROOT_PATH."classes/dao/products.class.php");
		$products = new Products($this->store_id);
		$listItem = $products->getObjects(1, "`cat_id` = '".$this->id."' and `status`='1' and `home`='1'",array("position"=>"DESC"),5);
	...
	function getUrl($lang='vn') {
		$url = '';
	    $url = "/$lang/san-pham/".$this->slug.'-c'.$this->id.'.html';
		return $url;
