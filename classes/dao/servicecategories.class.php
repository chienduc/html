...
include_once(ROOT_PATH."classes/dao/servicecategoryinfo.class.php");
...
class ServiceCategories extends Model {
	...
	function ServiceCategories($store_id = 0, $database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
    ...
    *-----------------------------------------------------------------------*/
	function getObject($value = '0', $key = 'id', $condition = '1>0') {
		...
		if($result) {
			$object = new ServiceCategoryInfo
			(	$result[0]['slug'],
				$result[0]['name'],
				$result[0]['keyword'],
				$result[0]['sapo'],
	...
	function getObjects($page = 1, $condition = '1>0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$objects[] = new ServiceCategoryInfo
				(	$result['slug'],
					$result['name'],
					$result['keyword'],
					$result['sapo'],
...
# Return ProductCategory from provided parent_id
    ...
    *------------------------------------------------------------------------*/
	...
	function generateCombo($value='', $noroot = 0) {
		global $amessages;
		$combo = '';
		if(!$noroot) $combo = '<option value="0"'.($value=='0'?" selected":"").'>G?c</option>';
	...
	function generateListbox($value=array()) {
		$combo='';
		$results = $this->select('id,name',"`store_id` = '".$this->store_id."' AND parent_id = '0' and `status`='1'",array("position"=>'ASC'));
		if($results) {
			$i=0;
			foreach($results as $key => $result) {
				$combo .= "<option value='".$result['id']."'".(in_array($result['id'],$value)?" selected":"").">".$result['name']."</option>";
				$s1results = $this->select('id,name',"`store_id` = '".$this->store_id."' AND parent_id = '".$result['id']."' and `status`='1'");
				if($s1results) {
					foreach($s1results as $key1 => $result1) {
						$combo .= "<option value='".$result1['id']."'".(in_array($result1['id'],$value)?" selected":"").">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;l--".$result1['name']."</option>";


						$s1resultss = $this->select('id,name',"`store_id` = '".$this->store_id."' AND parent_id = '".$result1['id']."' and `status`='1'");

						if($s1resultss) {
							foreach($s1resultss as $key2 => $result2) {
								$combo .= "<option value='".$result2['id']."'".(in_array($result2['id'],$value)?" selected":"").">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;l--".$result2['name']."</option>";
							}
						}
					}
				}
	...
	function getParentCategory($id){
		...
		if($id){
			...
			$articleCates = new ServiceCategories($this->store_id);
	...
	function getAllId($id=''){
		...
		if($id){
			$results = $this->select('id',"`store_id` = '".$this->store_id."' AND parent_id = '".$id."'");
			foreach($results as $key => $result) {
				$allId[]=$result['id'];
				$s1results = $this->select('id',"`store_id` = '".$this->store_id."' AND parent_id = '".$result['id']."'");
				if($s1results) {
					foreach($s1results as $key1 => $result1) {
						$allId[]=$result1['id'];
						$s2results = $this->select('id',"`store_id` = '".$this->store_id."' AND parent_id = '".$result1['id']."'");
						if($s2results){
							foreach($s2results as $key2 => $result2) {
								$allId[]=$result2['id'];
							}
