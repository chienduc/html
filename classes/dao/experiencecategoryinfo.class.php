...
class ExperienceCategoryInfo {
	...
	# Constructor
	function ExperienceCategoryInfo($slug, $name, $keyword, $sapo, $position, $viewed, $properties, $status, $store_id = 0, $parent_id = 0, $id = 0)
	{
		$this->id = $id;
	...
	function getUrl($lang='vn',$page = 1, $keywords = '', $sort_key = 'position', $sort_direction = 'asc') {
		...
		if(URL_TYPE == 1 || $page > 1) {	# Query string
			...
			$url = "/$lang/experiences/".$this->slug.'-c'.$this->id.($page>1?'-p'.$page:'').'.html';			
			return $url;
