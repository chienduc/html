...
class FengshuiInfo {
	...
	# Constructor
	function FengshuiInfo($slug, $title, $keyword, $sapo, $detail,$viewed, $date_created, $date_update, $position, $properties, $status, $home, $cat_id = 0, $store_id = 0, $id = 0)
	{
		$this->id = $id;
	...
	function getCatSlug() {
		...
		$fengshuiCategories = new FengshuiCategories($this->store_id);
		return $fengshuiCategories->getSlugFromId($this->cat_id);
	}
	function getCatName() {
		include_once(ROOT_PATH."classes/dao/fengshuicategories.class.php");
		$fengshuiCategories = new FengshuiCategories($this->store_id);
		return $fengshuiCategories->getNameFromId($this->cat_id);
	}
	function getSlug() {
		return $this->slug;		
	}
	...
	function getUrl($lang='vn') {
		...
		if(URL_TYPE == 1 || $page > 1) {	# Query string
			$url = "/$lang/".SCRIPT.'?act=fengshui&id='.$this->id;
			return $url;
		} elseif(URL_TYPE == 2) {	# SEO
			 $url = "/$lang/fengshuis/".$this->getCatSlug().'/'.$this->slug.'-'.$this->id.'.htm';
			return $url;
		} else return '';	
	}
}	
