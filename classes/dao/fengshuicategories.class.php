...
include_once(ROOT_PATH."classes/dao/fengshuicategoryinfo.class.php");

class FengshuiCategories extends Model {
	var $table;
	var $_db;
	var $store_id;
	
	function FengshuiCategories($store_id = 0, $database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		} else $this->_db = $database;
		$this->table = DB_PREFIX."fengshui_categories";
...
*-----------------------------------------------------------------------*/
	function getObject($value = '0', $key = 'id', $condition = '1>0') {
		...
		if($result) {
			$object = new FengshuiCategoryInfo
						(	$result[0]['slug'],
							$result[0]['name'],
							$result[0]['keyword'],
							$result[0]['sapo'],
	...
	function getObjects($page = 1, $condition = '1>0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$objects[] = new FengshuiCategoryInfo
								(	$result['slug'],
									$result['name'],
									$result['keyword'],
									$result['sapo'],
...
*-----------------------------------------------------------------------*/	
	...
	function cleanTrash() {
		...
		if($results) {
			...
			$fengshuis = new Fengshuis($this->store_id);
			# Loop all DELETED categories
			foreach($results as $key => $result) {
				# Change status of all Fengshuis in each category to DELETED too
				$fengshuis->update(array('status' => S_DELETED),"`store_id` = '".$this->store_id."' AND `cat_id` = '".$result['id']."'");
			}	
		}
	...
	# Return a FengshuiCategory Id from provided ID
	...
	# Return a FengshuiCategory Name from provided slug
	...
	# Return a FengshuiCategory slug from provided ID
	...
	# Return a FengshuiCategory name from provided ID
