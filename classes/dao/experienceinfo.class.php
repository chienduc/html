...
class ExperienceInfo {
	...
	# Constructor
	function ExperienceInfo($slug, $title, $keyword, $sapo, $detail,$viewed, $date_created, $date_update, $position, $properties, $status, $home, $cat_id = 0, $store_id = 0, $id = 0)
	{
		$this->id = $id;
	...
	function getCatSlug() {
		...
		$experienceCategories = new ExperienceCategories($this->store_id);
		return $experienceCategories->getSlugFromId($this->cat_id);
	}
	function getCatName() {
		include_once(ROOT_PATH."classes/dao/experiencecategories.class.php");
		$experienceCategories = new ExperienceCategories($this->store_id);
		return $experienceCategories->getNameFromId($this->cat_id);
	}
	function getSlug() {
		return $this->slug;		
	}
	...
	function getUrl($lang='vn') {
		...
		if(URL_TYPE == 1 || $page > 1) {	# Query string
			$url = "/$lang/".SCRIPT.'?act=experience&id='.$this->id;
			return $url;
		} elseif(URL_TYPE == 2) {	# SEO
			 $url = "/$lang/experiences/".$this->getCatSlug().'/'.$this->slug.'-'.$this->id.'.htm';
			return $url;
		} else return '';	
	}
}	
