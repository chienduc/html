...
include_once(ROOT_PATH.'classes/dao/addoninfo.class.php');
...
class Addons extends Model {
	...
	function Addons($store_id = 0,$database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
...
*-----------------------------------------------------------------------*/
	...
	function getAddonFromEvent($event = '') {
		include_once(ROOT_PATH.'classes/dao/events.class.php');
		$events = new Events();
		$eventId = $events->getIdFromName($event);
		if(!$eventId) return '';
		$sort = array('position'=>'ASC');
		$results = $this->select('name', "`store_id` = '".$this->store_id."' AND event='$eventId' AND status='1'", $sort,0,999);
		if($results) {
			$objects = array();
			foreach($results as $key => $result) {
				$objects[] = $result['name'];
