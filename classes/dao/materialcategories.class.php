...
include_once(ROOT_PATH."classes/dao/materialcategoryinfo.class.php");
...
class MaterialCategories extends Model {
	...
	function MaterialCategories($store_id = 0, $database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		} else $this->_db = $database;
		$this->table = DB_PREFIX."material_categories";
...
*-----------------------------------------------------------------------*/
	function getObject($value = '0', $key = 'id', $condition = '1>0') {
		...
		if($result) {
			$object = new MaterialCategoryInfo
						(	$result[0]['slug'],
							$result[0]['name'],
							$result[0]['keyword'],
							$result[0]['sapo'],
	...
	function getObjects($page = 1, $condition = '1>0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$objects[] = new MaterialCategoryInfo
								(	$result['slug'],
									$result['name'],
									$result['keyword'],
									$result['sapo'],
...
*-----------------------------------------------------------------------*/	
	...
	# Return a ProductCategory name from provided ID
	function getIdBusiness($id='0') {
		global $amessages;
		if(!$id) return '';
		$result = $this->select('id_business',"`store_id` = '".$this->store_id."' AND id = '$id'");
		if($result) return $result[0]['id_business'];
		return '';
	}
