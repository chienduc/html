...
include_once(ROOT_PATH.'classes/dao/woodshopinfo.class.php');

class Woodshop extends Model {
	var $table;
	var $_db;
	var $store_id;
	
	function Woodshop($store_id = 0, $database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		} else $this->_db = $database;
		$this->table = DB_PREFIX."woodshop";	
	...
	function getObjects($page = 1, $condition = '1>0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$woodshopInfos[] = new WoodshopInfo (	
				                            $result['name'],
											$result['status'],
											$result['position'], 
											$result['viewed'],
