...
Coder: PhanTom
**************************************************************************/
$templateFile = 'experiences.tpl.html';
...
$cateObject = $experienceCategories->getObject($cId,'id');
...
if($cateObject){
	if(!$slug || in_array($slug,array('0','index',$cateObject->getSlug()))) {
		...
		$rowsPages = $experiences->getNumItems('id', $condition,$article_per_page);
		$template->assign('rowsPages',$rowsPages);
		if($page < 1) $page = 1;
		if($page > $rowsPages['pages']) $page = $rowsPages['pages'];
		$listNew = $experiences->getObjects($page,$condition,$sort,$article_per_page);
...
# Show name module
$template->assign('nameModule',$messages['experiences']);
?>