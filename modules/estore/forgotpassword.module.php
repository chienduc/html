...
$templateFile = "forgotpassword.tpl.html";
...
if($_POST){
	...
	if($check == 0){
		...
		if (($isTel == 1) and ($isEmail == 1)){
			$username = $store_users->getUsernameFromEmail("email = '$email' "); 
			$update = $store_users->updateData(array('properties' => $time),$username,'username');
			$url = "http://"."$subDomain".".".DOMAIN."/index.php?op=estore&act=forgotpassword&plus=changepass&addon=$username";
			$url1 = "http://"."$subDomain".".".DOMAIN."/forgotpassword/changepass/$username".".html";

			# Send mail
			$To          = strip_tags($email);
			$from   = 'quang.ta@derasoft.com';
			$Subject     =$messages["newpass_email_title"];
			$body = sprintf($messages["newpass_email_body"],$username,$url1,$url1,$url,$url);
