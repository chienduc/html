...
$topNav = array($amessages['dash_board'] => '/'.ADMIN_SCRIPT.'?op=dashboard',
				...
				$amessages['manage_woodshop'] => '/'.ADMIN_SCRIPT.'?op=manage&act=woodshop',
				$amessages['add_new'] => '');
...
if($_POST && $request->element('doo') == 'submit') { # if form is submitted
	...
	if($validate['invalid']) {	# data input is not in valid form
		...
		# Category combo box
		$categoryCombo = $woodshopCategories->generateCombo($request->element('wid'),1);
		if($categoryCombo) $template->assign('categoryCombo',$categoryCombo);
		...
		if(!$validate['invalid']) {
			...
			# Operation tracking
			$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['add_woodshop'],$newId),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=woodshop&mod=list&wid=".$request->element('wid')."&rcode=6");
		}
	}
}
