...
# Category combo box
$categoryCombo = $experienceCategories->generateCombo($request->element('cat_id'),1);
...
if($_POST && $request->element('doo') == 'submit') { # if form is submitted
	...
	if($validate['invalid']) {	# data input is not in valid form
		...
		# Category combo box
		$categoryCombo = $experienceCategories->generateCombo($request->element('cat_id'));
		if($categoryCombo) $template->assign('categoryCombo',$categoryCombo);
	} else { # Valid data input
		# check duplicate experience name
		if($estore->getProperty('check_duplicate_experience_title')) {
			if($experiences->checkDuplicate($request->element('title'),'title',"cat_id = '".$request->element('cat_id')."'")) {
		...
		$dup = 1;
		while($dup) {
			$dup = $experiences->checkDuplicate($slug.($i?'-'.$i:''),'slug',"cat_id = '".$request->element('cat_id')."'");
			if($dup) $i++;
		}
		$slug .= $i?'-'.$i:'';
		...
		if(!$validate['invalid']) {
			...
			$newId = $experiences->addData($data);
					
			# Operation tracking
			$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['add_experience'],$request->element('title')),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=experience&mod=list&pId=".$request->element('cat_id')."&rcode=6");
		}
	}
}
