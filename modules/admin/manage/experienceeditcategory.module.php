...
$categoryInfo = $experienceCategories->getObject($id);
if(!$categoryInfo) {
	...
	if($request->element('doo') == 'delAvatar') {
		...
		if($categoryInfo->getProperty('avatar')) {
			...
			$experienceCategories->updateData($data,$id);
			$categoryInfo = $experienceCategories->getObject($id);
		}
	}
	...
	if($_POST && $request->element('doo') == 'submit') { # if form is submitted
		...
		if($validate['invalid']) {	# data input is not in valid form
			...
			# Category combo box
			$categoryCombo = $experienceCategories->generateCombo($request->element('parent_id',0));
			if($categoryCombo) $template->assign('categoryCombo',$categoryCombo);
			# Get list of custom fields by Thai Nguyen
			$fieldList = $fields->getObjects(1,"`status`='1' AND `module`='experienceCategories'",array('position' => 'ASC'));
			...
			if($experienceCategories->checkDuplicate($request->element('name'),'name',"`id` <> '$id'")) {
			...
			if($experienceCategories->checkDuplicate($slug,'slug',"`id` <> '$id'")) {
			...
			if(!$validate['invalid']) {
				...
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['edit_experience_category'],$experienceCategories->getNameFromId($id)),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
				
				# Redirect to editing page
				header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=experience&mod=editcategory&lang=$lang&id=$id&rcode=7");
			}
		}
		...
		# Category combo box
		$categoryCombo = $experienceCategories->generateCombo($categoryInfo->getParentId());
		if($categoryCombo) $template->assign('categoryCombo',$categoryCombo);
		# Get list of custom fields by Thai Nguyen
		$fieldList = $fields->getObjects(1,"`status`='1' AND `module`='experienceCategories'",array('position' => 'ASC'));
		if($fieldList) $template->assign('fieldList',$fieldList);
