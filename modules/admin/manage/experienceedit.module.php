...
if(!$experienceInfo) {
	...
	if($request->element('doo') == 'delPhoto') { 
		$experienceInfo = $experiences->getObject($id);
		$properties = $experienceInfo->getProperties();
		foreach($properties['photos'] as $key => $value) {
			if($value == $request->element('photo')) {
				...
				$experiences->updateData($data,$id);
				$experienceInfo = $experiences->getObject($id);
				break;
			}
		}
	...
	if($request->element('doo') == 'delAvatar') {
		$experienceInfo = $experiences->getObject($id);
		$properties = $experienceInfo->getProperties();
		if($experienceInfo->getProperty('avatar')) {
			unlink($gallery_path.$experienceInfo->getProperty('avatar'));
	...
	if($_POST && $request->element('doo') == 'submit') { # if form is submitted
		...
		$fieldList = $fields->getObjects(1,"`status`='1' AND `module`='experience'",array('position' => 'ASC'));
		...
		if($validate['invalid']) {	# data input is not in valid form
			...
			$experienceInfo = $experiences->getObject($id);
			$template->assign('itemInfo',$experienceInfo);
			
			# Category combo box
			$categoryCombo = $experienceCategories->generateCombo($request->element('cat_id',0));
			...
			if($estore->getProperty('check_duplicate_experience_name')) {
				if($experiences->checkDuplicate($request->element('name'),'name',"`id` <> '$id' AND `cat_id` = '".$request->element('cat_id')."'")) {
			...
			$dup = 1;
			while($dup) {
				$dup = $experiences->checkDuplicate($slug.($i?'-'.$i:''),'slug',"`id` <> '$id' AND `cat_id` = '".$request->element('cat_id')."'");
				if($dup) $i++;
			}
			$slug .= $i?'-'.$i:'';
			...
			if(!$validate['invalid']) {
				...
				if($experienceInfo) {			
					...
					# Operation tracking
					$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['edit_experience'],$request->element('title')),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
					
					# Redirect to editing page
					header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=experience&mod=edit&lang=$lang&id=$id&rcode=7");
				}
			}
		}
	} else { # Load experience category information to edit
		$template->assign('item',$experienceInfo);
		
		# Category combo box
		$categoryCombo = $experienceCategories->generateCombo($experienceInfo->getCatId());
		if($categoryCombo) $template->assign('categoryCombo',$categoryCombo);
