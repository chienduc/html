...
# Link
$link = '/'.ADMIN_SCRIPT."?op=manage&act=product&mod=listcategory&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=$page";
...
if($_POST) {
	switch($do) {
		...
		case 'sethome':
			...
			if($id) {
				...
				$result_code = 7;
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['set_home_product'],$listCategory),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));;
			} 
			break;
		case 'deletehome':
			$userInfo->checkPermission('pro_cat','edit');
	...
	header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=product&mod=listcategory&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pg=$page&ecode=$error_code&rcode=$result_code&pId=$pId");
} else {
