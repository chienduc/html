...
$topNav = array($amessages['dash_board'] => '/'.ADMIN_SCRIPT.'?op=dashboard',
				...
				$amessages['add_material_category'] => '');
...
# Category combo box
$categoryCombo = $materialCategories->generateCombo($request->element('pId'));
...
if($_POST && $request->element('doo') == 'submit') { # if form is submitted
	...
	if($validate['invalid']) {	# data input is not in valid form
		...
		if($materialCategories->checkDuplicate($request->element('name'))) {
		...
		$dup = 1;
		while($dup) {
			$dup = $materialCategories->checkDuplicate($slug.($i?'-'.$i:''),'slug');
			if($dup) $i++;
		}
		$slug .= $i?'-'.$i:'';
		...
		if(!$validate['invalid']) {
			...
			$newId=$materialCategories->addData($data);
			
			
			foreach($request->element('list_business') as $list){
				  $data = array('id_cate' => $newId,
						        'id_business' => $list );
			        $materialCate->addData($data);  
			    }
			# Operation tracking
			$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['add_material_category'],$request->element('name')),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=material&mod=listcategory&pId=".$request->element('parent_id')."&rcode=6");
		}
	}
}
