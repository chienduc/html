<?php
/*************************************************************************
Adding fengshui category module
----------------------------------------------------------------
BiDo Project
...
$topNav = array($amessages['dash_board'] => '/'.ADMIN_SCRIPT.'?op=dashboard',
				...
				$amessages['add_fengshui_category'] => '');
...
# Category combo box
$categoryCombo = $fengshuiCategories->generateCombo($request->element('pId'));
...
if($_POST && $request->element('doo') == 'submit') { # if form is submitted
	...
	if($validate['invalid']) {	# data input is not in valid form
		...
		if($fengshuiCategories->checkDuplicate($request->element('name'))) {
		...
		$dup = 1;
		while($dup) {
			$dup = $fengshuiCategories->checkDuplicate($slug.($i?'-'.$i:''),'slug');
			if($dup) $i++;
		}
		$slug .= $i?'-'.$i:'';
		...
		if(!$validate['invalid']) {
			...
			$fengshuiCategories->addData($data);
			# Operation tracking
			$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['add_fengshui_category'],$request->element('name')),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=fengshui&mod=listcategory&pId=".$request->element('parent_id')."&rcode=6");
		}
	}
}
