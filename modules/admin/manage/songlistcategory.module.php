...
$topNav = array($amessages['dash_board'] => '/'.ADMIN_SCRIPT.'?op=dashboard',
				...
				$amessages['manage_song'] => '/'.ADMIN_SCRIPT.'?op=manage&act=song',
				$amessages['list_song_category'] => '');
...
if($pId) {
	...
	$topNav[$amessages['list_song_category']] = '/'.ADMIN_SCRIPT.'?op=manage&act=song&mod=listcategory';
...
$url = '/'.ADMIN_SCRIPT."?op=manage&act=song&mod=listcategory&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=%d";
...
# Link
$link = '/'.ADMIN_SCRIPT."?op=manage&act=song&mod=listcategory&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=$page";
	...
	switch($do) {
		...
		case 'changegroup':
			...
			if($ids) {
				$listCategory = '';
				foreach ($ids as $id) {
					$productCategories->changePId($id,$parent_id);
					$listCategory .= ($listCategory?',&nbsp;':'').$productCategories->getNameFromId($id);
				}
				$result_code = 4;
				$pId = $parent_id;
		...
		case 'changeposition':
			...
			break;
		case 'cancel':		
			header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=song&mod=listcategory&lang=$lang&ecode=7&pId=$pId");
			exit;
			break;
	}
	header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=song&mod=listcategory&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pg=$page&ecode=$error_code&rcode=$result_code&pId=$pId");
} else {
