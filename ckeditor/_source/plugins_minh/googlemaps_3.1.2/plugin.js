...
http://code.google.com/p/gmaps-api-issues/issues/detail?id=3775
edit overlays
http://code.google.com/p/gmaps-api-issues/issues/detail?id=3755

To Do:
move end script to external file.
...
GoogleMapsHandler.prototype  = {
	...
	BuildEndingScript: function()
	{
		...
		aScript.push('function initLoader(e)');
		aScript.push('{');
		aScript.push('	if (e) ');
		aScript.push('	{');
		aScript.push('		var obj = e.target || e.srcElement;');
		aScript.push('		if (obj) obj.onclick=null;');
		aScript.push('	}');
		aScript.push('');

		aScript.push('	if (window.google && google.load)');
		aScript.push('	{');
		aScript.push('		if (window.google.maps)');
		aScript.push('			initializeMaps();');
		aScript.push('		else');
		aScript.push('			loadMaps();');
		aScript.push('		return;');
		aScript.push('	}');

		aScript.push('	var script = document.createElement("script");');

		aScript.push('	script.src = "//maps.googleapis.com/maps/api/js?sensor=false&libraries=geometry&callback=initializeMaps";');
		aScript.push('	script.type = "text/javascript";');
		aScript.push('	document.getElementsByTagName("head")[0].appendChild(script);');
		aScript.push('}');

		aScript.push('function loadMaps()');
		aScript.push('{');
		aScript.push('	google.load("maps", "3", {"callback":initializeMaps,other_params:"sensor=false"});');
		aScript.push('}');

		aScript.push('');
		aScript.push('if (!window.gmapsLoaders) gmapsLoaders = [];');
		aScript.push('');

		for (i = 0; i < maps.length; i++)
			aScript.push('gmapsLoaders.push(CreateGMap' + maps[i]  + ');');

		aScript.push('');
		aScript.push('function initializeMaps()');
		aScript.push('{');
		aScript.push('	InitializeLabels();');
		aScript.push('	for(var i=0; i<gmapsLoaders.length; i++)');
		aScript.push('		gmapsLoaders[i]();');
		aScript.push('}');
		aScript.push('');


		aScript.push('function CKEMap(div, options)');
		aScript.push('{');
		...
		aScript.push('		zoom: options.zoom,');
		aScript.push('		center: new google.maps.LatLng(options.center[0], options.center[1]),');
		aScript.push('		mapTypeId: allMapTypes[ options.mapType ],');
		aScript.push('		scaleControl: options.scaleControl,');
		aScript.push('		overviewMapControl: options.overviewMapControl,');
		aScript.push('		overviewMapControlOptions: options.overviewMapControlOptions');
		aScript.push('	};');

		// None, Default, Small, Big
		aScript.push('switch (options.navigationControl)');
		aScript.push('{');
		aScript.push('case "None":');
		aScript.push('	myOptions.navigationControl = false;');
		aScript.push('	break;');
		aScript.push('case "Default":');
		aScript.push('	myOptions.navigationControl = true;');
		aScript.push('	break;');
		aScript.push('case "Small":');
		aScript.push('	myOptions.navigationControl = true;');
		aScript.push('	myOptions.navigationControlOptions = {style: google.maps.NavigationControlStyle.SMALL};');
		aScript.push('	break;');
		aScript.push('case "Full":');
		aScript.push('	myOptions.navigationControl = true;');
		aScript.push('	myOptions.navigationControlOptions = {style: google.maps.NavigationControlStyle.ZOOM_PAN};');
		aScript.push('	break;');
		aScript.push('}');

		aScript.push('switch (options.mapsControl)');
		aScript.push('{');
		aScript.push('case "None":');
		aScript.push('    myOptions.mapTypeControl = false;');
		aScript.push('		break;');
		aScript.push('case "Default":');
		aScript.push('    myOptions.mapTypeControl = true;');
		aScript.push('		break;');
		aScript.push('case "Full":');
		aScript.push('    myOptions.mapTypeControl = true;');
		aScript.push('    myOptions.mapTypeControlOptions = {style: google.maps.MapTypeControlStyle.HORIZONTAL_BAR};');
		aScript.push('		break;');
		aScript.push('case "Menu":');
		aScript.push('    myOptions.mapTypeControl = true;');
		aScript.push('    myOptions.mapTypeControlOptions = {style: google.maps.MapTypeControlStyle.DROPDOWN_MENU};');
		aScript.push('		break;');
		aScript.push('}');

		aScript.push('	this.map = new google.maps.Map(div, myOptions);');
		aScript.push('	return this;');
		aScript.push('}');


		aScript.push('CKEMap.prototype.AddMarkers = function(aPoints)');
		aScript.push('{');
		aScript.push('	function createMarker(map, data) {');
		aScript.push('		var clickable=(data.text!=""),');
		aScript.push('			point=new google.maps.LatLng(data.lat, data.lon),');

		aScript.push('			marker = new google.maps.Marker( {position:point, map:map, title:data.title, icon: "//maps.gstatic.com/mapfiles/ms/micons/" + data.color + "-dot.png", clickable:clickable});');
		aScript.push('		if (data.title)');
		aScript.push('			new Label({ position:point, text:marker.title, map:map});');

		aScript.push('		if (clickable) google.maps.event.addListener(marker, "click", function() {');
		aScript.push('			if (!marker._infoWindow)');
		aScript.push('				marker._infoWindow =  new google.maps.InfoWindow({');
		aScript.push('			  content: "<div class=\\\"InfoContent\\\">" + data.text + "</div>",');
		aScript.push('					maxWidth:data.maxWidth');
		aScript.push('				});');

		aScript.push('			 marker._infoWindow.open(map, marker);');
		aScript.push('		});');

		aScript.push('	}');

		aScript.push('	for (var i=0; i<aPoints.length; i++)');
		aScript.push('		createMarker(this.map, aPoints[i]);');
		aScript.push('};');

		aScript.push('CKEMap.prototype.AddTexts = function(aPoints)');
		aScript.push('{');
		aScript.push('	for (var i=0; i<aPoints.length; i++)');
		aScript.push('	{');
		aScript.push('	var data=aPoints[i];');
		aScript.push('	var point=new google.maps.LatLng(data.lat, data.lon);');
		aScript.push('	if (data.title)');
		aScript.push('		new Label({ position:point, text: "<span>" + data.title + "</span>", map: this.map, className:data.className});');
		aScript.push('	}');
		aScript.push('};');


		aScript.push('CKEMap.prototype.AddLines = function( aLines )');
		aScript.push('{');
		aScript.push('	for (var i=0; i<aLines.length; i++)');
		aScript.push('	{');
		aScript.push('		var line = aLines[i];');
		aScript.push('		var clickable = (line.text!="");');
		aScript.push('		var l = new google.maps.Polyline( {map:this.map, path: google.maps.geometry.encoding.decodePath(line.points), strokeColor: line.color, strokeOpacity: line.opacity, strokeWeight: line.weight, clickable:clickable} );');
	...
	*/
		...
		aScript.push('CKEMap.prototype.AddAreas = function( aAreas )');
		aScript.push('{');
		aScript.push('	for (var i=0; i<aAreas.length; i++)');
		aScript.push('	{');
		aScript.push('		var area = aAreas[i];');
		aScript.push('		var line = area.polylines[0];');
		aScript.push('		var clickable = (area.text!="");');

		aScript.push('		var a = new google.maps.Polygon( {map:this.map, paths: google.maps.geometry.encoding.decodePath(line.points), strokeColor: line.color, strokeOpacity: line.opacity, strokeWeight: line.weight, fillColor:area.color, fillOpacity:area.opacity, clickable:clickable} );');
...
//		aScript.push('		});');
		...
		aScript.push('CKEMap.prototype.AddKml = function( url )');
		aScript.push('{');
		aScript.push('	new google.maps.KmlLayer(url, {map:this.map});');
		aScript.push('};');

		for (i = 0; i < maps.length; i++)
		{
			if (this.getMap( maps[i] ).generatedType==3)
			{
				autoload = true;
				break;
			}
		...
		if (autoload)
		{
			...
			aScript.push('    window.addEventListener("load", initLoader, false);');
			aScript.push('} else {');
			aScript.push('    window.attachEvent("onload", initLoader);');
			aScript.push('}');
		}
	...
	validateDOM: function()
	{
		...
		for(i=0; i<aMaps.length; i++)
		{
			...
			else
			{
				...
				if (div)
				{
					element = CKEDITOR.dom.element( div );
					element.remove( true );
					modified = true;
				}
				delete maps[ mapId ];
...
CKEGoogleMap.prototype.generateStaticMarkers = function()
{
	...
	for (; i<this.markerPoints.length; i++)
	{
		...
		aPoints.push('&markers=color:' + point.color + '|' + point.lat + ',' + point.lon );
	}
	return aPoints.join('');
};
...
// v <= 1.5
	...
	// AddMarkers( [{lat:37.45088, lon:-122.21123, text:'Write your text'}] );
		while( (result = regexpMarkers.exec(script)) )
		{
			...
			if (result[7])
				maxWidth = result[7];
			this.markerPoints.push( {lat:result[1], lon:result[2], text:this.decodeText(result[4]), color:color, title:this.decodeText(title), maxWidth:maxWidth} );
		}
...
//		aScript.push('	AddLines( map, [' + aLines.join(',\r\n') + '] );');
	...
	//	AddAreas( map, [{polylines: [{color:'#008000', opacity:0.7, weight:2, points:'iatcFnzrhVpfA{nOtuExkJnd@ffOwbIecJ', levels:'BBBBB'}], fill: true, color:'#008000', opacity:0.2, outline:true}] );
			...
			{
				lines = [{color:result[1], weight:result[3], opacity:result[2], points:this.decodeText(result[4])} ];

				maxWidth = 200;
				if (result[13])
					maxWidth = result[13];
				this.areasData.push( {polylines: lines, fill:true, color:result[9], opacity:result[10], text:this.decodeText(result[12]), maxWidth:maxWidth } );
			}
		}
		else
		{
			...
			while( (result = regexpLines.exec(script)) )
			{
				this.linesData.push( {color:result[2], weight:result[3], opacity:result[4], points:this.decodeText(result[6]), text:this.decodeText(result[8]), maxWidth:result[9]} );
			}

			while( (result = regexpAreas.exec(script)) )
			{
				lines = [{color:result[2], opacity:result[3], weight:result[4], points:this.decodeText(result[6])} ];
				this.areasData.push( {polylines:lines, fill:true, color:result[9], opacity:result[10], text:this.decodeText(result[12]), maxWidth:result[13] } );
			}
...
CKEGoogleMap.prototype.BuildScript = function()
{
	...
	if (this.markerPoints.length)
	{
		...
		{
			point = this.markerPoints[i];
			aPoints.push('{lat:' + point.lat + ', lon:' + point.lon + ', text:"' + this.encodeText(point.text) + '",color:"' + point.color + '", title:"' + this.encodeText(point.title) + '", maxWidth:' + point.maxWidth + '}');
		}
...
CKEDITOR.plugins.add( 'googlemaps',
{
	requires : [ 'dialog' ],
	// translations
	lang : ['cs', 'el', 'en', 'es', 'fi', 'it', 'nl', 'pl', 'sk', 'tr'],
		...
		editor.ui.addButton( 'GoogleMaps',
			{
				...
				command : 'googlemaps',
				icon : this.path + 'images/mapIcon.gif'
			} );
			...
			editor.contextMenu._.listeners.unshift( function( element, selection )
				{
					...
					// Set here the trick
					element.setAttribute( '_cke_realelement', true );
					return null;
				});
			...
			editor.contextMenu.addListener( function( element, selection )
				{
					...
					// Now we clear it
					element.removeAttribute( '_cke_realelement' );
