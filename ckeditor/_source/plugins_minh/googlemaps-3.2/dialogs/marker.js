...
CKEDITOR.dialog.add( 'googlemapsMarker', function( editor )
{
	...
	return {
		...
		setValues : function(data)
		{
			...
			markerImage = getIcon( data.color );
			document.getElementById( markerIcon ).src = markerImage;
		},
								...
								onClick : function (evt)
									{
										...
										if (targetName == 'img')
										{
											editor.openNestedDialog( 'googlemapsIcons', function(dialog)
												{
													dialog.initialSrc = markerImage;
													dialog.onSelect = function(src) { markerImage= src; document.getElementById( markerIcon ).src = markerImage;};
												}, null);
										}
									},
