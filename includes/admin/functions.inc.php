...
# Created by PhanTom
function resizes($image_path,$thumb_path,$image_name,$thumb_name,$nws,$nhs,$quality=90) 
{ 
	$type = strtolower(substr($image_name,-3));
	switch ($type) {
		case 'jpg':
	...
	$dst = imagecreatetruecolor($nws,$nhs);
    imagecopyresampled($dst,$src,0,0,$src_x,$src_y,$nws,$nhs,$ow,$oh); 
	imagejpeg($dst, "$thumb_path/$thumb_name",$quality);
	imagedestroy($src);
	imagedestroy($dst);	
    return true; 
...
function isImage($str) {
	$ext = strtolower(substr($str,-3));
	if(preg_match("/jpg|png|bmp|gif/",$ext)) return 1;
	return 0;
}
